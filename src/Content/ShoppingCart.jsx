...
export default function Cart({data}) {
    ...
    const sub = () => {
        setCounter((prev) => {
            return (counter > 1 ? prev - 1 : prev)
        })
        setnumberPrice((prev) => {
            return (counter > 1 ? prev - Number(data.numberPrice) : prev)
        })
        setDiscount((prev) => {
            return (counter > 1 ? prev - Number(data.numberPrice) : prev)
        })
    ...
    return (
        <Grid container sx={{alignItems: 'center'}}>
            ...
            <Grid item xs={12} sm={1} sx={{display: 'flex', justifyContent: 'center'}} paddingTop={1}>
                <svg xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" fill='#808080' width="17"
                     height="20">
                    <path
